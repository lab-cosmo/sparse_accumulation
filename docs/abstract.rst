...
.. math::
    ...
    \text{Output}[..., \mu] = \sum_{m_1, m_2} \text{X_1}[..., m_1] * \text{X_2}[..., m_2] * C_{m_1, m_2, \mu},

where :math:`C_{m_1, m_2, \mu}` are the Clebsch-Gordan coefficients. These coefficients are very sparse, particularly for the complex-valued version the sparsity pattern is that the only non-zero values are when :math:`m_1 + m_2 = \mu`. For the real-valued version, the sparsity pattern is more complicated but still very sparse. Thus, it makes sense to store only non-zero values in a one-dimensional array. In this case, one needs to provide additional arrays with indices telling the corresponding :math:`m_1`, :math:`m_2` and :math:`\mu`. With such data organization, the CG iteration falls to the defined above sparse accumulation operation. 
