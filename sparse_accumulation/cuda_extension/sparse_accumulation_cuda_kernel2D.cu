...
template <typename scalar_t>
__global__ void sparse_accumulation_cuda_forward_kernel2D(
    scalar_t* __restrict__ output,
    const scalar_t* __restrict__ X1,
    const scalar_t* __restrict__ X2,
    const int64_t* __restrict__ idx_output,
    ...
    int delta_now_X1 = i * X1_third_size ;//j * X1_third_size + i * ny * X1_third_size;
    int delta_now_output = i * output_size; //j * output_size + i * ny * output_size;
    int delta_now_X2 = i * X2_third_size; //j * X2_third_size + i * ny * X2_third_size;

   
    //int delta_buffer_X1 = (BLOCK_SIZE * threadIdx.x + threadIdx.y) * X1_third_size;
    //int delta_buffer_X2 = (BLOCK_SIZE * threadIdx.x + threadIdx.y) * X2_third_size;
    int delta_buffer_X1 = threadIdx.x * X1_third_size;
    int delta_buffer_X2 = threadIdx.x * X2_third_size;
...
template <typename scalar_t>
__global__ void sparse_accumulation_cuda_backward_kernel2D(
    scalar_t* __restrict__ d_X1,
    scalar_t* __restrict__ d_X2,
    const scalar_t* __restrict__ d_output,
    const scalar_t* __restrict__ X1,
 ...
 *      output_tensor[nsample,nfeatures,output_size]: output Tensor
 */
std::vector<torch::Tensor> sparse_accumulation_cuda_forward2D(
    torch::Tensor X1,
    torch::Tensor X2,
    torch::Tensor idx_output,
    int output_size,
  ...
  auto output = torch::zeros({X1.sizes()[0], output_size},
            torch::TensorOptions()
            .dtype(X1.dtype())
            .device(X1.device()));

  auto X1_third_size = X1.sizes()[1];
  auto X2_third_size = X2.sizes()[1];
  const auto batch_sizex = output.sizes()[0];
  const auto batch_sizez = output.sizes()[1];
  ...
  AT_DISPATCH_FLOATING_TYPES(output.type(), "sparse_accumulation_forward_cuda", ([&] {
      ...
      sparse_accumulation_cuda_forward_kernel2D<<<grid_dim, block_dim, total_buf_size>>>(
          output.data_ptr<scalar_t>(),
          X1.data_ptr<scalar_t>(),
          X2.data_ptr<scalar_t>(),
          idx_output.data_ptr<int64_t>(),
...
std::vector<torch::Tensor> sparse_accumulation_cuda_backward2D(
    torch::Tensor d_output,
    torch::Tensor X1,
    torch::Tensor X2,
    torch::Tensor idx_output,
...
std::vector<torch::Tensor> sparse_accumulation_gpu_forward(
    ...
    if (X1.dim()==2 && X2.dim()==2){
        return sparse_accumulation_cuda_forward2D(X1,X2,idx_output,output_size,idx_1,idx_2,multipliers);      
    }else{
...
std::vector<torch::Tensor> sparse_accumulation_gpu_backward(
    ...
    if (X1.dim()==2 && X2.dim()==2){
        return sparse_accumulation_cuda_backward2D(d_output,X1,X2,idx_output,idx_1,idx_2,multipliers);
    }else{
