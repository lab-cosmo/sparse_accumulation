import torch
import sparse_accumulation_active_dim_first_cpp, sparse_accumulation_active_dim_middle_cpp
...
def accumulate_active_dim_middle(X1, X2, idx_output, output_size, idx_1, idx_2, multipliers):
    tensors = [X1, X2, idx_output, idx_1, idx_2, multipliers]
    check_all_on_same_device(tensors)
    
    if X1.is_cuda:
        raise NotImplementedError("active dimensions other than the last do not have a CUDA implementation")
    else:
        return SparseAccumulationCPUMiddle.apply(X1, X2, idx_output, output_size, idx_1, idx_2, multipliers) 
    
    
class SparseAccumulationCPUMiddle(torch.autograd.Function):
    @staticmethod
...
def accumulate_active_dim_first(X1, X2, idx_output, output_size, idx_1, idx_2, multipliers):
    tensors = [X1, X2, idx_output, idx_1, idx_2, multipliers]
    check_all_on_same_device(tensors)
    ...
    if X1.is_cuda:
        ...
        return SparseAccumulationCPUFirst.apply(X1, X2, idx_output, output_size, idx_1, idx_2, multipliers) 
    
    
class SparseAccumulationCPUFirst(torch.autograd.Function):
    @staticmethod
    def forward(ctx, X1, X2, idx_output, output_size, idx_1, idx_2, multipliers):
        tensors = [X1, X2, idx_output, idx_1, idx_2, multipliers]
        check_all_on_cpu(tensors)
        ...
        output = sparse_accumulation_active_dim_first_cpp.forward_contiguous(X1, X2, idx_output, output_size, idx_1, idx_2, multipliers)
        ctx.save_for_backward(*[X1, X2, idx_output, idx_1, idx_2, multipliers])
        return output
    ...
    def backward(ctx, grad_output):
        ...
        d_X1, d_X2 = sparse_accumulation_active_dim_first_cpp.backward_contiguous(grad_output, X1, X2, idx_output, idx_1, idx_2, multipliers)
